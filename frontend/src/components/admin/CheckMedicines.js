...
import { getAllMedicines, deleteMedicines, changeMedicine } from '../apicalls/auth/eventcalls';
...
function CheckMedicines() {
    ...
    const  getManagers=()=>{
        getAllMedicines(user._id,token).then(user=>{
            if(user){
                console.log(user);
                setManagers(user);
    ...
    const changeStatus=(Eventid)=>{
        changeMedicine(Eventid,user._id,token).then(data=>{
if(data){
    if(data.message){
    ...
    const deleteEvent=(id)=>{
        deleteMedicines(id,user._id,token).then(res=>{
        ...
        return(
            ...
            {managers?.map((evnts,idx)=>{
                return(
                    ...
                    <li class="list-group-item "> Medicine Name: <span className="badge badge-danger mr-3"> {evnts?.medicinename} </span> </li>
                    <li class="list-group-item "> Pincode: <span className="badge badge-danger mr-3"> {evnts?.pincode}  </span> </li>
                    <li class="list-group-item "> Event Status: <span className="badge badge-danger mr-3"> {evnts?.active?<p>true</p>:<p>false</p>}</span> </li>
