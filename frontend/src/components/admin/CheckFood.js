...
import { getAllFood, changeFood } from '../apicalls/auth/eventcalls';
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import Base from '../basic/Base'
import { deleteFood } from '../apicalls/auth/eventcalls'
...
function CheckFood() {
    ...
    const  getManagers=()=>{
        getAllFood(user._id,token).then(user=>{
            if(user){
                console.log(user);
                setManagers(user);
    ...
    const changeStatus=(Eventid)=>{
        changeFood(Eventid,user._id,token).then(data=>{
if(data){
    if(data.message){
    ...
    const deleteEvent=(id)=>{
        deleteFood(id,user._id,token).then(res=>{
